class hdo::users {
  ...
  $users = {
    ...
    'nikolai'  => { email => 'nikolai@lucida.no',       keytype => 'ssh-ed25519', key => 'AAAAC3NzaC1lZDI1NTE5AAAAIJnf+1ksW/bDU2KjKguAZS5emOev9TbH/423C6M/dhf9' }
  }
  ...
  @ssh_authorized_key { "hdo+${users[nikolai][email]}":
    ensure => present,
    user   => $hdo::params::user,
    type   => $users[nikolai][keytype],
    key    => $users[nikolai][key],
    tag    => 'hdo-deployer'
  }
  ...
  admin_account { 'nikolai':  config => $users[nikolai] }
